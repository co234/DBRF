...
class train_model(object):
    ...
    def training(self):
        ...
        outputs_train = model(self.input_data['x'],self.input_data['s'])
        outputs_test = model(self.target_data['x'],self.target_data['s'])
        ...
        print('DBRF+LR TEST' + ': ACC %.2f DP %.2f' % (acc*100, dp))
    ...
    def train_epoch_progress(self, epoch):
        ...
        for batch in self.train_iter:
            ...
            w = 1-torch.max(pred_b, 1)[0]

            # constrain_loss = (1-self.beta)*(tyy_entropy).mean()-(self.beta - self.xi)*tyb_entropy.mean()+self.beta*tyyb_entropy 
            ...
            lp = supervision_loss*w

            vae_loss = vae_recon_loss+vae_kld + self.gamma*vae_tc_loss

            loss = vae_loss + constrain_loss + lp.mean()
    ...
    def evaluate(self):
        ...
        print("PREDICTED POSITIVE RATIO: {:.2f}".format(pred_check*100))

        print('DBRF*   TEST' + ': ACC %.2f DP %.2f' % (acc*100, dp))
        return acc, dp
